...
class Train:
    ...
    def get_training_data(self):
        if not os.path.isdir("data/training_data/") or len(os.listdir("data/training_data/")) == 0:
            while True:
                ...
                if download_data == "y":
                    ...
                    subprocess.Popen([dir+'download_training_data.sh %s' % dir], shell=True).wait()
    ...
    def setup_train_test_sets(self):
        data_loader = CustomDatasetDataLoader(self.opt)
        training_dataset, validation_dataset = data_loader.split_dataset()

        self.dataset_train = data_loader.create_dataloader(training_dataset, shuffle_batches=not self.opt.serial_batches)
        self.dataset_val = data_loader.create_dataloader(validation_dataset, shuffle_batches=False)
        self.dataset_train_size = len(training_dataset)
        self.dataset_val_size = len(validation_dataset)
    ...
    def display_terminal_train(self, iter_start_time, i_epoch, i_train_batch, visuals_flag):
    ...
    def display_terminal_test(self, epoch_num, average_test_results, test_time):
        self.tb_visualizer.print_current_validate_errors(
            epoch_num, average_test_results, test_time)

    def display_visualizer_test(self, test_epoch_visuals, epoch_num, average_test_results, total_steps):
        self.tb_visualizer.plot_scalars(
    ...
    def test(self, i_epoch, total_steps):
        ...
        self.display_terminal_test(i_epoch, average_test_results, test_time)
        if self.opt.no_save:
