...
def get_plane_distance_batched(verts_hand, plane):
    point_1 = torch.FloatTensor(([0, 0, plane[0, 3]/plane[0, 2], 1])).unsqueeze(0).unsqueeze(0).cuda()
    point_1 = point_1.repeat(verts_hand.shape[0], 1, 1)
    plane = plane.repeat(verts_hand.shape[0], 1)
    dot = torch.einsum("bij,bj->bi", torch.cat((verts_hand,
                                                torch.ones((verts_hand.shape[0], verts_hand.shape[1], 1)).cuda()), -1) - point_1, plane)
    return (dot > 0).float(), dot
