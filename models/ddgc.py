...
class Model(BaseModel):
    ...
    def create_and_init_networks(self):
        ...
        if len(self.gpu_ids) > 1:
            self.image_encoder = torch.nn.DataParallel(self.image_encoder, device_ids=self.gpu_ids)
    ...
    def create_grasp_generator(self):
        return NetworksFactory.get_by_name(
            'grasp_generator', input_dim=3 + 3)  # 3 for rotation 3 for tranlsation
    ...
    def create_and_init_optimizer(self):
        self.current_lr_image_encoder = self.opt.lr_G
        # initialize optimizers
        if self.opt.optimizer == "Adam":
            optimizer = torch.optim.Adam
            self.optimizer_image_enc = optimizer(
                self.image_encoder.parameters(),
                lr=self.current_lr_image_encoder,
            ...
            self.optimizer_grasp_generator = optimizer(self.grasp_generator.parameters(),
                                                       lr=self.current_lr_image_encoder, momentum=0.9)
            if not self.opt.no_discriminator:
                self.current_lr_discriminator = self.opt.lr_D
    ...
    def init_losses(self):
        ...
        self.loss_g_plane = Variable(self.Tensor([0]))
    ...
    def set_input(self, data_input):
        self.input_rgb_img = data_input['rgb_img'].float().permute(
            0, 3, 1, 2).contiguous()
        self.input_mask_img = data_input['mask_img'].float().permute(0, 3, 1, 2)
        self.input_plane_eq = data_input['plane_eq'].float()
        ...
        if torch.cuda.is_available():
            ...
            self.input_mask_img = self.input_mask_img.to(self.device)
            self.input_plane_eq = self.input_plane_eq.to(self.device)
        ...
        if self.is_train:
            ...
            if train_generator:
                ...
                if self.gradient_accumulation_current_step % self.gradient_accumulation_every == 0:
                    self.optimizer_image_enc.step()
                    self.optimizer_grasp_generator.step()
                    self.optimizer_image_enc.zero_grad()
    ...
    def forward_G(self, train=True):
        ...
        mask = self.input_mask_img
        input_img = torch.cat((input_img, mask), 1)

        img_representations = self.image_encoder.forward(
            input_img)
        ...
        hand_configuration, R, T = self.grasp_generator.forward(img_representations, random_rots, translation_in)
        ...
        self.delta_HR = hand_configuration.abs().mean()
    ...
    def calculate_generator_losses(self, points_3d, points_3d_area, R, T, translation_in, hand_configuration_unconstrained):
        self.calculate_interpenetration_loss(points_3d, points_3d_area)
        # CONTACT LOSS ON OPTIMIZED HAND
        self.calculate_contact_loss(points_3d)
        self.calculate_orientation_loss(R, T)
        self.calculate_plane_loss(points_3d)
        if not self.opt.no_discriminator:
            self.calculate_discriminator_loss(R, hand_configuration_unconstrained, T, translation_in)

    def calculate_interpenetration_loss(self, points_3d, points_3d_area):
        interpenetration = torch.FloatTensor([0]).to(self.device)
    ...
    def calculate_contact_loss(self, points_3d):
    ...
    def calculate_orientation_loss(self, R, T):
    ...
    def calculate_plane_loss(self, points_3d):
        plane_interp = torch.FloatTensor([0]).to(self.device)
        for i in range(self.batch_size):
            above_plane, distance = util.get_plane_distance(
                points_3d[i], self.input_plane_eq[i])
            if (above_plane == 0).sum() == 0:
                continue
            plane_interp = plane_interp + distance[above_plane == 0].mean()

        self.loss_g_plane = -1 * self.opt.lambda_G_plane * plane_interp / self.batch_size

    def calculate_discriminator_loss(self, R, hand_configuration_unconstrained, T, translation_in):
    ...
    def combine_generator_losses(self):
        ...
        if not self.opt.no_plane_loss:
            combined_losses += self.loss_g_plane[0]
        if not self.opt.no_discriminator:
    ...
    def forward_D(self):
        ...
        HR, R, T = self.grasp_generator.forward(img_representations,
    ...
    def get_current_errors(self):
        ...
        losses.append(('g plane loss', self.loss_g_plane.cpu().data.numpy()))
        if not self.opt.no_discriminator:
    ...
    def get_current_scalars(self):
        scalars = [
            ...
            ('lr G', self.current_lr_image_encoder)
        ]
        if not self.opt.no_discriminator:
    ...
    def save(self, label, epoch):
        ...
        self.save_network(self.image_encoder, 'image_encoder', label, epoch)
        self.save_optimizer(self.optimizer_image_enc, 'image_encoder', label)
    ...
    def load(self):
        ...
        # load image_encoder
        self.load_network(self.image_encoder, 'image_encoder', load_epoch, self.device)
        ...
        if self.is_train:
            # load optimizers
            self.load_optimizer(self.optimizer_image_enc, 'image_encoder',
    ...
    def update_learning_rate(self):
        ...
        self.current_lr_image_encoder -= lr_decay_G
        if not self.opt.no_discriminator:
            lr_decay_D = self.opt.lr_D / self.opt.nepochs_decay
            self.current_lr_discriminator -= lr_decay_D
        self.set_learning_rate(plot=False)
        print('update image_encoder and grasp_generator learning rate: %f -> %f' %
              (self.current_lr_image_encoder + lr_decay_G, self.current_lr_image_encoder))
        if not self.opt.no_discriminator:
    ...
    def set_learning_rate(self, plot=True):
        for param_group in self.optimizer_image_enc.param_groups:
            param_group['lr'] = self.current_lr_image_encoder
        ...
        if plot:
            print('set image_encoderand grasp_generator learning rate to: %f' %
                  (self.current_lr_image_encoder))
            if not self.opt.no_discriminator:
